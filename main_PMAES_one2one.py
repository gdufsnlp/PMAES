...
from utils.read_data import read_pos_vocab, read_essays_single_score_for_one2one
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source2target', type=str, default='one2one', help='Setting of source-target pair')

    parser.add_argument('--source_id', type=int, default=1, help='source prompt id')
    parser.add_argument('--target_id', type=int, default=2, help='source prompt id')
    ...
    target_prompt_id = args.target_id
    seed = args.seed

    seed_all(seed)

    print("Test prompt id is {} of type {}".format(target_prompt_id, type(target_prompt_id)))
    print("Seed: {}".format(seed))
    configs = Configs()

    data_path = configs.DATA_PATH
    train_path = data_path + str(target_prompt_id) + '/train.pk'
    dev_path = data_path + str(target_prompt_id) + '/dev.pk'
    test_path = data_path + str(target_prompt_id) + '/test.pk'
    ...
    source_target_pair = [args.source_id, args.target_id]
    print('source-2-target pair:', source_target_pair)
    pos_vocab = read_pos_vocab(read_configs)
    train_data, valid_data, test_data = read_essays_single_score_for_one2one(read_configs, pos_vocab, args.attribute_name, source_target_pair)
    ...
    for e_index in range(1, epochs+1):
        ...
        TrainSingleOverallScoring(args,
                                  ...
                                  target_prompt_id, e_index,
                                  tr_log, args.attribute_name)
    file_time = time.strftime("%Y-%m-%d", time.localtime())
    result_file = 'result/one2one/PMAES-{}.txt'.format(seed)
