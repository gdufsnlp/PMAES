...
if __name__ == '__main__':
    ...
    parser.add_argument('--source2target', type=str, default='many2many', help='Setting of source-target pair')
    parser.add_argument('--source_ids', type=list, default=[1, 2], help='source prompt ids')
    parser.add_argument('--target_ids', type=list, default=[3, 4], help='set random seed')
    ...
    source2target = 's' + ''.join([str(ids) for ids in args.source_ids]) + '-' + 't' + ''.join([str(ids) for ids in args.target_ids])
    seed = args.seed

    seed_all(seed)

    print("Test prompt id is {} of type {}".format(source2target, type(source2target)))
    print("Seed: {}".format(seed))
    configs = Configs()

    data_path = 'data/many2many/'
    train_path = data_path + str(source2target) + '/train.pk'
    dev_path = data_path + str(source2target) + '/dev.pk'
    test_path = data_path + str(source2target) + '/test.pk'
    ...
    for e_index in range(1, epochs+1):
        ...
        TrainSingleOverallScoring(args,
                                  ...
                                  args.target_ids, e_index,
                                  tr_log, args.attribute_name)
    file_time = time.strftime("%Y-%m-%d", time.localtime())
    result_file = 'result/many2many/PMAES-{}.txt'.format(seed)
    with open(result_file, 'a', encoding='utf-8') as f:
        f.write(file_time + 'TargetPrompt: {} Trait: {}'.format(args.target_ids, args.attribute_name) + '\n')
