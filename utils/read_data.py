...
def read_vocab(read_configs):
    file_path = read_configs['train_path']
    word_freq = {}
    total_words, unique_words = 0, 0
    with open(file_path, 'rb') as train_file:
        train_essays_list = pickle.load(train_file)
    for index, essay in tqdm(enumerate(train_essays_list), desc='read vocab from' + file_path):
        content = essay['content_text']
        content = text_tokenizer(content, True, True, True)
        content = [w.lower() for w in content]
        for word in content:
...
def read_essay_sets_single_score(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name):
    ...
    for essay in essay_list[:10]:
        essay_id = int(essay['essay_id'])
        essay_set = int(essay['prompt_id'])
        content = essay['content_text']
        if attribute_name in essay.keys():
            y = int(essay[attribute_name])
            out_data['score'].append([y])
            item_index = np.where(readability_features[:, :1] == essay_id)
            item_row_index = item_index[0][0]
            item_features = readability_features[item_row_index][1:]
            out_data['readability'].append(item_features)
            feats_df = normalized_features_df[normalized_features_df.loc[:, 'item_id'] == essay_id]
            feats_list = feats_df.values.tolist()[0][1:]
            out_data['linguistic'].append(feats_list)
            sent_tokens = text_tokenizer(content, replace_url_flag=True, tokenize_sent_flag=True)
            sent_tokens = [[w.lower() for w in s] for s in sent_tokens]
            ...
            for sent in sent_tokens:
                length = len(sent)
                if length > 0:
                    if out_data['max_sent_len'] < length:
                        out_data['max_sent_len'] = length
                    tags = nltk.pos_tag(sent)
                    for tag in tags:
                        if tag[1] in pos_tags:
                            tag_indices.append(pos_tags[tag[1]])
            ...
            out_data['essay_pos'].append(sent_tag_indices)
            out_data['prompt_ids'].append(essay_set)
            if out_data['max_sent_num'] < len(sent_tag_indices):
                out_data['max_sent_num'] = len(sent_tag_indices)
    assert(len(out_data['essay_pos']) == len(out_data['readability']))
    print(' Num of essay: {}'.format(len(out_data['essay_pos'])))
    return out_data


def read_essay_sets_single_score_for_one2one(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name, source_target_pair):
    out_data = {
        'essay_pos': [],
        'readability': [],
        'linguistic': [],
    ...
    for essay in essay_list[:100]:
        essay_id = int(essay['essay_id'])
        essay_set = int(essay['prompt_id'])
        content = essay['content_text']
        if essay_set not in source_target_pair:
            continue
        if attribute_name in essay.keys():
            y = int(essay[attribute_name])
            out_data['score'].append([y])
...
def read_essay_sets_single_score_for_single2multi(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name, need_id):
    out_data = {
        'pos_x': [],
        'readability_x': [],
        'features_x': [],
...
def read_essay_sets_single_score_for34(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name):
    out_data = {
        'pos_x': [],
        'readability_x': [],
        'features_x': [],
...
def read_essay_sets_single_score_for56(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name):
    out_data = {
        'pos_x': [],
        'readability_x': [],
        'features_x': [],
...
def read_essay_sets_single_score_for78(essay_list, readability_features, normalized_features_df, pos_tags, attribute_name):
    out_data = {
        'pos_x': [],
        'readability_x': [],
        'features_x': [],
...
def read_essays_single_score_for3456(read_configs, pos_tags, attribute_name):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for3456(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    dev_data = read_essay_sets_single_score_for3456(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    test_data = read_essay_sets_single_score_for3456(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    return train_data, dev_data, test_data


def read_essays_single_score_for_one2one(read_configs, pos_tags, attribute_name, source_target_pair):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for_one2one(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, source_target_pair)
    dev_data = read_essay_sets_single_score_for_one2one(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, source_target_pair)
    test_data = read_essay_sets_single_score_for_one2one(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, source_target_pair)
    return train_data, dev_data, test_data


def read_essays_single_score_for_single2multi(read_configs, pos_tags, attribute_name, source_id, target_ids):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for_single2multi(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, source_id)
    dev_data = read_essay_sets_single_score_for_single2multi(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, source_id)
    test_data = read_essay_sets_single_score_for_single2multi(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name, target_ids)
    return train_data, dev_data, test_data


def read_essays_single_score_for34(read_configs, pos_tags, attribute_name):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for34(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    dev_data = read_essay_sets_single_score_for34(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    test_data = read_essay_sets_single_score_for34(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    return train_data, dev_data, test_data


def read_essays_single_score_for56(read_configs, pos_tags, attribute_name):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for56(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    dev_data = read_essay_sets_single_score_for56(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    test_data = read_essay_sets_single_score_for56(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    return train_data, dev_data, test_data


def read_essays_single_score_for78(read_configs, pos_tags, attribute_name):
    readability_features = get_readability_features(read_configs['readability_path'])
    linguistic_features = get_linguistic_features(read_configs['features_path'])
    normalized_linguistic_features = get_normalized_features(linguistic_features)
    with open(read_configs['train_path'], 'rb') as train_file:
    ...
    with open(read_configs['test_path'], 'rb') as test_file:
        test_essays_list = pickle.load(test_file)
    train_data = read_essay_sets_single_score_for78(
        train_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    dev_data = read_essay_sets_single_score_for78(
        dev_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    test_data = read_essay_sets_single_score_for78(
        test_essays_list, readability_features, normalized_linguistic_features, pos_tags, attribute_name)
    return train_data, dev_data, test_data
